...
const panelFallback = (
  <div className="text-muted-foreground text-base" role="status">
    Loading…
  </div>
);
...
const TABS: { id: SideTab; label: string; icon: React.ReactNode }[] = [
  { id: 'rides', label: 'Rides', icon: <Layers className="size-5 shrink-0" aria-hidden /> },
  { id: 'shop', label: 'Shop', icon: <ShoppingBag className="size-5 shrink-0" aria-hidden /> },
  { id: 'visitors', label: 'Guests', icon: <Users className="size-5 shrink-0" aria-hidden /> },
  { id: 'stats', label: 'Stats', icon: <BarChart2 className="size-5 shrink-0" aria-hidden /> },
];
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden md:flex-row">
        <main className="relative flex min-h-0 flex-1 flex-col overflow-hidden p-5">
          ...
          <div className="relative mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h1 className="font-heading text-foreground text-xl leading-none font-black sm:text-2xl">
            ...
            <p className="text-muted-foreground max-w-lg text-sm leading-snug sm:text-right">
            ...
            <div className="grid grid-cols-2 gap-3 pb-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              ...
              {rides.length === 0 && (
                <div className="text-muted-foreground col-span-full flex flex-col items-center justify-center py-16">
                  ...
                  <div className="mt-1 text-base">Buy a ride from the Shop to get started!</div>
                </div>
              )}
        ...
        <aside className="bg-sidebar border-sidebar-border flex w-full shrink-0 flex-col overflow-hidden border-t md:w-96 md:border-t-0 md:border-l">
            ...
            >
              {TABS.map((tab) => (
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
                  className="font-heading data-[state=active]:after:bg-primary relative flex-1 flex-col gap-1 py-3 text-xs tracking-widest uppercase"
                ...
                >
                  <span className="relative inline-flex flex-col items-center gap-0.5">
                    ...
                    {tab.id === 'rides' && brokenCount > 0 && (
                      <Badge variant="destructive" className="absolute -top-1 -right-2 h-5 min-w-5 px-1 text-xs">
                        {brokenCount}
                      </Badge>
                    )}
                  </span>
                    ...
                    <ScrollArea className="h-full max-h-[calc(100vh-8rem)]">
                      <div className="space-y-3 p-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          <Card
                            size="sm"
                            className="gap-1 border-red-500/20 bg-red-500/10 py-3 text-center shadow-none"
                          >
