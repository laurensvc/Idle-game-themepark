...
export const useGameStore = create<GameState & GameActions>((set, get) => ({
  ...
  tick: () => {
    ...
    const updatedRides = state.rides.map(ride => {
      ...
      if (updated.status === 'operating') {
        ...
        const hasIncomeBoost = state.purchasedUpgrades.some(
          id => getUpgradeDefinition(id)?.rideId === ride.definitionId && getUpgradeDefinition(id)?.effect === 'income_boost',
        )
        ...
        const hasCapacityBoost = state.purchasedUpgrades.some(
          id => getUpgradeDefinition(id)?.rideId === ride.definitionId && getUpgradeDefinition(id)?.effect === 'capacity_boost',
        )
        const capacityBoost = hasCapacityBoost ? state.purchasedUpgrades
          .map(id => getUpgradeDefinition(id))
          .filter(u => u?.rideId === ride.definitionId && u?.effect === 'capacity_boost')
          .reduce((sum, u) => sum + (u?.value ?? 0), 0) : 0
        ...
        if (Math.random() < effectiveBreakdownChance && updated.ticksSinceLastBreakdown > 30) {
          ...
          newNotifications.push(
            createNotification('breakdown', `${def.name} has broken down!`, ride.instanceId),
          )

          // Auto repair if purchased
          const hasAutoRepair = state.purchasedUpgrades.some(
            id => getUpgradeDefinition(id)?.rideId === ride.definitionId && getUpgradeDefinition(id)?.effect === 'auto_repair',
          )
          if (hasAutoRepair) {
        ...
        if (updated.repairProgress >= 100) {
          ...
          updated.repairProgress = 0
          newNotifications.push(
            createNotification('repair', `${def.name} is back online!`, ride.instanceId),
          )
        }
      }
    ...
    if (state.isAutoCleanEnabled) {
      ...
      updatedRides.forEach(r => { r.dirtLevel = Math.max(0, r.dirtLevel - 1) })
    }
    ...
    const newHappiness = Math.max(0, Math.min(100,
      state.parkHappiness + (happinessTarget - state.parkHappiness) * 0.05,
    ))

    // Spawn new visitor groups occasionally
    const newVisitors: VisitorGroup[] = [...state.visitors]
    if (state.tick % 5 === 0 && operatingRides > 0) {
      const visitorAttraction = state.purchasedUpgrades
        .map(id => getUpgradeDefinition(id))
        .filter(u => !u?.rideId && u?.effect === 'visitor_attraction')
        .reduce((sum, u) => sum + (u?.value ?? 0), 0)
    ...
    set({
      rides: state.rides.map(r =>
        r.instanceId === instanceId
          ? { ...r, status: 'repairing', repairProgress: 0, isAutoRepair: false }
          : r,
      ),
      notifications: [
